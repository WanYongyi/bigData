# -*- coding: utf-8 -*-
"""
Created on Mon Jun  3 20:22:47 2024
...
'''
项集：一个集合，在篮子里的一个产品即为一个项，若干项的集合称为项集
{X -> y} 关联规则左侧的项集称为先决条件 

frozenset({'其他蔬菜', '根茎类蔬菜'}) --> frozenset({'全脂牛奶'}) 支持度 0.023183 置信度： 0.48927 lift值为： 1.914833
支持度：在所有项集中{X,y}同时出现的概率
把大于支持度的项集保留下来称为频繁项集
置信度：（条件概率），在发生X的条件下发生y的概率
提升度（lift）：含有X的条件下同时含有y的可能性与没有这个条件下项集中含有y的可能性之比
'''
...
def calSupport(D, Ck, min_support):
    # 计算每个项集的支持度
    # relist： 满足最小置信度的一元商品项集，supportData： 对应的支持度
    dict_sup = {}
    for i in D:
        for j in Ck:
            # 这里判断j是否i的子集不能用in，但不知道为什么（待解决）
            if j.issubset(i):
    ...
    relist = []
    for i in dict_sup:
        # 这里计算的是   出现某个商品的购物车的数量  /  购物车总数  ，就是包含该商品项集的支持度
        # dict_sup[i]指代商品购物车数量
...
def apriori(dataSet, minSupport=0.2):
    ...
    C1 = createC1(dataSet)
    D = list(map(set, dataSet))  # 使用list()转换为列表， D中元素是集合
    # print(len(D))
...
def calcConf(freqSet, H, supportData, ruleList, minConf=0.7):
    # 计算置信度
    '''
    从二项集开始计算，不用一项集开始是因为自己无法指向自己；
    如果置信度（min_support）设置为0， 那三项集的个数应该是  C31 * C32 * 2 个
    '''
    for conseq in H:  # 遍历H中的所有项集并计算它们的可信度值
        conf = supportData[freqSet] / \
            supportData[freqSet - conseq]  # 可信度计算，结合支持度数据
        # 提升度lift计算lift = p(a & b) / p(a)*p(b)
        lift = supportData[freqSet] / \
            (supportData[conseq] * supportData[freqSet - conseq])
...
def gen_rule(L, supportData, minConf=0.7):
    ...
    for i in range(1, len(L)):  # 从二项集开始计算
        for freqSet in L[i]:  # freqSet为所有的k项集
            # 求该k项集的所有非空子集，1项集，2项集，直到k-1项集，用H1表示，为list类型,里面为frozenset类型，
            H1 = list(freqSet) # 强制转换为list
            all_subset = []
